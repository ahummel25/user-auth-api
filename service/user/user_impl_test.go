...
func TestLogin(t *testing.T) {
	t.Run("successful authentication", func(t *testing.T) {
		...
		mockColl.On("UpdateOne", ctx, updateFilter, mock.AnythingOfType("primitive.M")).Return(&mongo.UpdateResult{}, nil)
	...
	t.Run("update last login date fails", func(t *testing.T) {
		...
		updateFilter := bson.M{"user_id": user.UserID}
		mockColl.On("UpdateOne", ctx, updateFilter, mock.AnythingOfType("primitive.M")).Return(nil, errors.New("update failed"))
...
func TestCreateUser(t *testing.T) {
	docMatcher := mock.MatchedBy(func(doc interface{}) bool {
		...
		requiredFields := []string{"user_id", "email", "user_name", "password", "first_name", "last_name", "role", "creation_date", "last_update_date"}
